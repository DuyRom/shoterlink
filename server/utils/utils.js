...
function getShortURL(address, domain) {
  // Nếu domain không tồn tại, sử dụng DEFAULT_DOMAIN trực tiếp
  const base = domain || env.DEFAULT_DOMAIN;
  // Loại bỏ dấu / cuối nếu có
  const link = `${base.replace(/\/$/, '')}/${address}`;
  // Nếu base đã chứa giao thức (http:// hoặc https://), không thêm protocol
  const url = /^https?:\/\//.test(base) ? link : `http://${link}`;
  return { address, link, url };
}
